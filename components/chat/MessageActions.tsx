...
export function MessageActions({
  ...
  const recordInteraction = async (type: string, action: "add" | "remove" = "add") => {
    if (!messageId) return;
  ...
  const handleCopy = async () => {
    try {
      ...
      await recordInteraction("copy", "add");
      // Mark copy as selected locally without affecting like/dislike
      setUserInteractions((prev) => (
        prev.includes("copy") ? prev : [...prev, "copy"]
      ));
    } catch (error) {
  ...
  const handleLike = () => {
    // Toggle like on/off and ensure dislike is not selected at the same time
    setUserInteractions((prev) => {
      const hasLike = prev.includes("like");
      const withoutDislike = prev.filter((t) => t !== "dislike");

      if (hasLike) {
        // Clicking again removes the like and updates the DB
        recordInteraction("like", "remove");
        return withoutDislike.filter((t) => t !== "like");
      }

      // Selecting like: record interaction and clear any dislike
      recordInteraction("like", "add");
      // Also clear any stored dislike
      return [...withoutDislike, "like"];
    });
  };

  const handleDislike = () => {
    // Toggle dislike on/off and ensure like is not selected at the same time
    setUserInteractions((prev) => {
      const hasDislike = prev.includes("dislike");
      const withoutLike = prev.filter((t) => t !== "like");

      if (hasDislike) {
        // Clicking again removes the dislike and updates the DB
        recordInteraction("dislike", "remove");
        return withoutLike.filter((t) => t !== "dislike");
      }

      // Selecting dislike: record interaction and clear any like
      recordInteraction("dislike", "add");
      return [...withoutLike, "dislike"]; 
    });
  };

  return (
    <div className="flex items-center gap-0 mt-1 -ml-2">
